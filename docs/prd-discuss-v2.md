# Product Requirements Document (PRD): Discuss v2

> **Changelog from v1:** Replaced Vercel with Coolify (self-hosted PaaS) as the hosting platform. Updated architecture diagram, technical decisions, middleware notes, SSL/subdomain configuration, infrastructure requirements, and success metrics accordingly.
...
**Tech stack:** Next.js (App Router), Supabase (Postgres + Auth + Storage), Stripe, Tiptap editor, ShadCN UI, Tailwind CSS, self-hosted on Coolify.
...
| FR-4 | Next.js Middleware must intercept all requests, extract the subdomain from the Host header, and route to the appropriate tenant context. Middleware runs as standard Node.js middleware on the Coolify-hosted server (not edge functions). |
...
│  │  ├── *.discusslabs.com → Next.js container    │   │
│  │  └── discusslabs.com → Next.js container      │   │
│  ├───────────────────────────────────────────────┤   │
...
|---|---|---|
| **Framework** | Next.js (App Router) | Server Components, Server Actions, Node.js Middleware |
...
| **Hosting** | Coolify (self-hosted PaaS) | Deployed as Docker container on VPS; Traefik for reverse proxy, wildcard SSL via Let's Encrypt DNS challenge, Git-push auto-deploy |
| **UI** | ShadCN + Tailwind | Copied components, full control |

### 7.3 Hosting & Infrastructure (Coolify)

Coolify is an open-source, self-hosted PaaS that runs on a VPS (e.g., Hetzner, DigitalOcean). It provides Vercel-like DX (Git push → auto-deploy) while giving full control over infrastructure and eliminating per-seat or per-request hosting fees.

#### 7.3.1 Deployment Model
- Next.js app is deployed as a **Docker container** via Coolify's Git integration (push to `main` → auto-build → deploy).
- Coolify uses **Nixpacks** or a custom **Dockerfile** to build the Next.js standalone output.
- The `next start` command runs the production server inside the container.

#### 7.3.2 Wildcard Subdomains & SSL
- **DNS:** A wildcard A record (`*.discusslabs.com`) and a root A record (`discusslabs.com`) must point to the VPS IP address.
- **Reverse proxy:** Coolify uses **Traefik** as its built-in reverse proxy. Traefik routes `*.discusslabs.com` and `discusslabs.com` to the Next.js container.
- **SSL:** Wildcard SSL certificates are provisioned via **Let's Encrypt** using the **DNS challenge** method (not HTTP challenge, which doesn't support wildcards). This requires configuring a supported DNS provider (e.g., Cloudflare API token) in Traefik's certificate resolver.
- **Important:** If using Cloudflare for DNS, the wildcard subdomain records must have proxy mode **disabled** (DNS-only / grey cloud) unless on Cloudflare's Business plan, which supports wildcard SSL proxying.

#### 7.3.3 Subdomain Routing
- Next.js **Middleware** (`middleware.ts`) runs on the Node.js server (not edge) and intercepts every request.
- Middleware extracts the subdomain from the `Host` header, resolves the tenant, and rewrites the request path internally (e.g., `acme.discusslabs.com/dashboard` → `/acme/dashboard`).
- No edge runtime is required — standard Node.js middleware is sufficient since all requests hit the same server.

#### 7.3.4 Infrastructure Requirements

| Resource | Recommendation | Notes |
|---|---|---|
| **VPS provider** | Hetzner, DigitalOcean, or similar | Cost-effective, reliable |
| **Minimum VPS specs** | 2 vCPU, 4 GB RAM, 80 GB SSD | Sufficient for MVP; scale vertically or add nodes later |
| **Estimated VPS cost** | ~$10–20/month | Significantly cheaper than Vercel Pro ($20/mo) with no per-request or bandwidth limits |
| **DNS provider** | Cloudflare (free tier) | Wildcard DNS support; API integration for Let's Encrypt DNS challenge |
| **Coolify version** | Latest stable (v4+) | Self-hosted on the same VPS or a separate management VPS |

#### 7.3.5 Operational Considerations
- **Monitoring:** Coolify provides basic container health monitoring. For production, integrate an external uptime monitor (e.g., UptimeRobot, Better Stack) and application-level monitoring (e.g., Sentry for errors).
- **Backups:** Coolify supports automated backups. Configure database backups separately via Supabase's built-in backup system.
- **Scaling:** For MVP, a single VPS is sufficient. For growth, Coolify supports multi-server deployments — add additional nodes and load-balance across them.
- **CI/CD:** Coolify watches the Git repo and auto-deploys on push to the configured branch. No separate CI/CD pipeline is needed for deployment, though a GitHub Actions pipeline should run tests and linting before merge.
- **Zero-downtime deploys:** Coolify supports rolling deployments via Docker health checks. Configure a health check endpoint (`/api/health`) in the Next.js app.
...
| **Page load speed** | < 2s initial, < 500ms navigation | Application-level analytics (e.g., Plausible, PostHog) |
| **Data isolation** | Zero cross-tenant leaks | Automated RLS tests, security audit |
| **Uptime** | 99.9% | External uptime monitor (e.g., UptimeRobot, Better Stack) + Supabase status |
...
| OQ-8 | Which VPS provider and region should be used for the initial Coolify deployment? | Affects latency for target user base |
| OQ-9 | Should Coolify run on the same VPS as the app, or on a separate management server? | Affects resource allocation and isolation |
