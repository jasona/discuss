...
5. **Perform Internal Research:** Use available tools and context to explore:
   - Existing docs in `/docs` (PRDs, CRDs, DRDs, task files) related to the topic
...
6. **Perform External Research:** When appropriate, use web search and documentation to gather:
   ...
   - **Platform and framework guidelines** (e.g., ASP.NET Core, Blazor, iOS/Android HIG, Material Design)
...
8. **Save RSD:** Save the generated document as `rsd-[project-name]-[version].md` inside the `/docs` directory.
...
6. What internal resources should be emphasized?
   A. Existing PRDs/CRDs/DRDs or docs in `/docs`
...
7. Any specific external research priorities?
   A. Framework/library best practices (e.g., ASP.NET Core, Blazor, .NET)
...
- **Existing Related Work (Internal)**
  - Relevant PRDs/CRDs/DRDs in `/docs`
